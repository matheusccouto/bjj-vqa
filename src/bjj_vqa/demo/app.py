...
def _accuracy_bar(value: float) -> str:
    """Render a small HTML accuracy bar."""
    pct = value * 100
    if pct >= 70:
        bar_color = "#16a34a"
    elif pct >= 50:
...
def _build_leaderboard(summaries: list[ModelSummary]) -> str:
    ...
    for i, s in enumerate(summaries):
        ...
        cat_cells = "".join(
            _accuracy_bar(s.by_category[c] if c in s.by_category else 0.0)
            for c in all_cats
        )
        sub_cells = "".join(
            _accuracy_bar(s.by_subject[sub] if sub in s.by_subject else 0.0)
            for sub in all_subs
        )
        rows.append(
            "<tr>"
    ...
    css_rules = (
        ...
        f".leaderboard th {{ background:{HEADER_COLOR}; color:white; "
...
def _image_to_html(image_path: str, data_dir: Path) -> str:
    """Convert an image file to an HTML img tag with base64 data URI."""
    img_path = data_dir / image_path
...
def _build_question_card(
    ...
    answer_text = (
        choices[answer_idx] if 0 <= answer_idx < len(choices) else "?"
    )
    ...
    if total_models:
        model_summary = f"{models_correct}/{total_models} correct"
    else:
        model_summary = "No results"
    ...
    with gr.Blocks(
        ...
        with gr.Tabs():
            ...
            with gr.TabItem("Leaderboard"):
                ...
                gr.Markdown(
                    "_Placeholder results — "
                    "real evaluation runs will replace these._"
                    if results_path is None
                    else "_Results loaded from eval log._",
                )
            ...
            with gr.TabItem("Browse Questions"):
                ...
                question_list = gr.HTML(
                    value="".join(
                        _build_question_card(s, summaries, data_dir)
                        for s in samples
                    ),
                )
                ...
                def _filter_questions(
                    ...
                    if level != "All":
                        filtered = [
                            s
                            for s in filtered
                            if s.get("experience_level") == level
                        ]
                    if query.strip():
                        ql = query.lower()
                        filtered = [
